    ...
    stages {
        ...
        stage('docker build') {
            steps{
                ...
                docker build -t rjonmarco/seccambackend:latest . --network=host --build-arg DEFINED_HOST=${HOSTURL} --build-arg DEFINED_PASSWORD=${MYSQLPASS} --build-arg DEFINED_DBNAME=${DBNAME}
                '''
            }
        }
